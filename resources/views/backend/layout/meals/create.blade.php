...
@section('content')
    <div class="page-header d-flex align-items-center justify-content-between">
        <div>
            <h1 class="page-title">Create Meal</h1>
        </div>
        <div class="ms-auto d-flex align-items-center gap-2">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="javascript:void(0);">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Create Meal</li>
            </ol>
        </div>
    </div>
                    ...
                    <form action="{{ route('meals.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" id="title" class="form-control"
                                value="{{ old('title') }}" placeholder="Meal title">
                            @error('title')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="text" name="time" id="time" class="form-control"
                                    value="{{ old('time') }}" placeholder="e.g. 15 min">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="servings" class="form-label">Servings</label>
                                <input type="text" name="servings" id="servings" class="form-control"
                                    value="{{ old('servings') }}" placeholder="e.g. 2">
                            </div>
                            <div class="col-md-3 mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            @php
                                $oldIngredients = old('ingredients', [['grocery_type_id' => '', 'title' => '', 'quantity' => '']]);
                                $groceryTypeOptions = $groceryTypes
                                    ->map(fn($type) => '<option value="' . $type->id . '">' . e($type->name) . '</option>')
                                    ->implode('');
                            @endphp
                                    ...
                                    <div class="ingredient-item border rounded p-2 mb-2">
                                        <div class="row g-2 align-items-end">
                                            <div class="col-md-4">
                                                ...
                                                <select name="ingredients[{{ $index }}][grocery_type_id]" class="form-control">
                                                    ...
                                                    @foreach ($groceryTypes as $type)
                                                        <option value="{{ $type->id }}" {{ (string) ($ingredient['grocery_type_id'] ?? '') === (string) $type->id ? 'selected' : '' }}>
                                                            {{ $type->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <input type="text" name="ingredients[{{ $index }}][title]" class="form-control" value="{{ $ingredient['title'] ?? '' }}" placeholder="Ingredient name">
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">Quantity</label>
                                                <input type="text" name="ingredients[{{ $index }}][quantity]" class="form-control" value="{{ $ingredient['quantity'] ?? '' }}" placeholder="e.g. 2 slices">
                                            </div>
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm" id="add-ingredient">+ Add Ingredient</button>
                            @error('ingredients')
